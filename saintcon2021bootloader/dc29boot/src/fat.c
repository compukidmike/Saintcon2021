...
void write_block(uint32_t block_no, uint8_t *data, bool quiet, WriteState *state) {
    ...
    if ((bl->flags & UF2_FLAG_NOFLASH) || bl->payloadSize != 256 || (bl->targetAddr & 0xff) ||
		...
		if(bl->targetAddr >= FLASH_MEMORY_OFFSET){
			//If this is the first block for the SPI flash, erase it before writing
			static bool flash_erased = false;
			if(flash_erased == false){
				flash_erase_all();
				flash_erased = true;
			}
			//This is data that belongs in the SPI flash
			flash_write(bl->targetAddr-FLASH_MEMORY_OFFSET, bl->data, 256);
		}
	} else {
